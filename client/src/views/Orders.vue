    ...
    <div v-else>
      ...
      <div class="card" v-if="submittedOrders.length">
        <div class="card-header">
          <h3 class="card-title">
            {{ t("orders.submittedOrders", { count: submittedOrders.length }) }}
          </h3>
        </div>
          ...
          <table class="orders-table">
            ...
            <tbody>
              <tr v-for="order in submittedOrders" :key="order.order_number">
                ...
                <td class="col-supplier">{{ getSuppliers(order) }}</td>
                <td class="col-items">
                ...
                <td class="col-date">
                  {{
                    t("orders.leadTimeDays", { count: order.lead_time_days })
                  }}
                </td>
                <td class="col-date">
                ...
                <td class="col-value">
                  <strong>{{
                    formatCurrency(order.total_value, currentCurrency)
                  }}</strong>
                </td>
              </tr>
            </tbody>
  ...
  setup() {
    ...
    const loadOrders = async () => {
      try {
        ...
        // Backend already returns submitted orders newest-first
    ...
    const getSuppliers = (order) => {
      return [...new Set(order.items.map((item) => item.supplier))].join(", ");
    };
